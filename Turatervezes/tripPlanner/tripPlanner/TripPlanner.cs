                            ...
                            try
                            {
                                ...
                                else
                                {
                                    ...
                                    for (int i = 0; i < oRs.RecordCount; i++)
                                    {
                                        ...
                                        if (!decimal.TryParse(tourStr, out tour))
                                        {
                                            tour = 1100; // Alapértelmezett érték, ha nem sikerül konvertálni
                                            SBO_Application.StatusBar.SetText($"Érvénytelen érték a(z) {i + 1}. sorban a tour_kg oszlopban", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
                                        }*/

                                        decimal cap;
                                        string capStr = oDT.Columns.Item("db_carkg").Cells.Item(i).Value?.ToString();
                                        if (!decimal.TryParse(capStr, out cap))
                                        {
                                            cap = 1000; // Alapértelmezett érték, ha nem sikerül konvertálni
                                            SBO_Application.StatusBar.SetText($"Érvénytelen érték a(z) {i + 1}. sorban a car_kg oszlopban", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Error);
                                        }
                                        if (oDT.Columns.Item("db_tourkg").Cells.Item(i).Value >= cap) {
                                            
                                            matrix.CommonSetting.SetRowBackColor(i, red);
                                            SBO_Application.StatusBar.SetText($"CommonSettng hiba, {oDT.Rows.Count} rows loaded", BoMessageTime.bmt_Short, BoStatusBarMessageType.smt_Success);
                    ...
                    case BoEventTypes.et_MATRIX_LINK_PRESSED:
                        {
                            try
                            {
                                if(pVal.ColUID == "order_id")
                                {
                                    ...
                                    SBO_Application.OpenForm(BoFormObjectEnum.fo_Order, "", ord);
                                }
                            }
                            catch(Exception ex)
                            {
